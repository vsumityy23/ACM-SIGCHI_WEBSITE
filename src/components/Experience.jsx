  ...
  return (
    <div className='experience bg-black w-screen text-white pt-4 pt-16 overflow-x-hidden' id='experience'>
      <div className='pt-12 sm:px-16 text-center'>
        <h2 className='text-4xl sm:text-5xl font-extrabold mt-2'>Upcoming Events</h2>
      </div>
      ...
      <VerticalTimeline className='mt-9'>
        {upcomingEvents.map((event, index) => (
          <VerticalTimelineElement
            ...
            contentStyle={{ background: "#151030", color: "#fff", borderRadius: "8px", padding: "20px" }}
            contentArrowStyle={{ borderRight: "7px solid  #232631" }}
            date={event.date}
            iconStyle={{ background: '#4A5568', color: '#fff', borderRadius: '50%', boxShadow: '0px 0px 10px rgba(0, 0, 0, 0.5)' }}
            icon={
              <a className='flex justify-center items-center w-full h-full' href={event.url} target='_blank' rel='noopener noreferrer'>
                <img
      ...
      <div className='pt-12 sm:px-16 text-center'>
        ...
        <h2 className='text-4xl sm:text-5xl font-extrabold mt-2'>Past Events</h2>
      </div>
      <VerticalTimeline className='mt-9'>
        {pastEvents.map((event, index) => (
          <VerticalTimelineElement
            key={index}
            className="relative vertical-timeline-element--work"
            contentStyle={{ background: "#2D2F3B", color: "#fff", borderRadius: "8px", padding: "20px" }}
            contentArrowStyle={{ borderRight: "7px solid  #232631" }}
            date={event.date}
            iconStyle={{ background: '#4A5568', color: '#fff', borderRadius: '50%', boxShadow: '0px 0px 10px rgba(0, 0, 0, 0.5)' }}
            icon={
          ...
          >
            <div>
              <h3 className='text-white text-[24px] font-bold'>{event.title}</h3>
              <p className='text-gray-400 text-[16px] font-semibold mb-10 pb-5' style={{ margin: 0 }}>
                {event.description}
